...
| `src/query.ts`（1729 行） | 轮次级 API 调用与工具执行 |
...
第二层（自动）：分类器
  → 仅在 --auto 模式下启用
  → TranscriptClassifier 分析历史判断是否允许
  → BashClassifier 专门处理 shell 命令安全性
...
```
当前 token 数 ≥ token 上限 × 85%  →  进入 WARNING 状态
连续多轮保持 WARNING             →  自动触发压缩
```
...
发一个专用的压缩请求给 Claude，请它生成结构化摘要：

```
系统提示：你是一个会话摘要助手
任务说明：以下是一段编程辅助对话的早期部分，生成简明摘要
...
```
每次 submitMessage() 开始时：
         ↓
① 扫描记忆文件（memoryScan）
② 根据当前对话内容判断相关性（findRelevantMemories）
   → 使用关键词匹配或向量相似度
③ 组装记忆摘要文本（buildMemorySystemMessage）：

   # 相关记忆
   ## User memories
   - 用户是 Go 专家，React 新手
   ## Feedback memories
   - 集成测试用真实数据库（原因：防止 mock 偏差）
   ## Project memories
   - 2026-05-20 后冻结合并
...
记忆不是全量注入的，而是**按相关性过滤**的。这避免了每次对话都塞入大量过时信息占用 token。同时，通过四种类型的划分，清晰区分了"用户是谁"（user）、"怎么工作"（feedback）、"做什么"（project）、"在哪里"（reference）四个维度。
...
对话历史以 JSONL 格式写到磁盘（`~/.claude/sessions/[session-id]/transcript.jsonl`）：
